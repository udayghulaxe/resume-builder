    ...
    return (
        <div className="resume-section resume-section-skills">
            ...
            {widgetData.items.map((item, index) => {
                return (
                    <Chip key={index} label={item.title} className={`resume-section-body skills-chip ${widgetData.rounded ? '' : 'no-rounded'}`} variant={widgetData.filled ? 'filled' : 'outlined'} />
                );
            })}
            </div>


            {openEditorName === props.componentItem.name ? <SkillsEditor setWidgetData={setWidgetData} open={open} setOpen={setOpen} componentColumn={props.componentColumn} componentName={props.componentItem.name} editorData={widgetData} /> : null }
        </div>
    );
}
