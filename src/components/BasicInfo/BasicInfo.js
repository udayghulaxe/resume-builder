		...
		<div className="resume-section resume-section-basic-info">
			...
			<div>
				<Grid mt={1}
					...
					{ widgetData.phone && <Grid item xs={6}>
						<Item>
							<PhoneIcon fontSize="15"></PhoneIcon>
							<Box component="span" sx={{ pl: 1 }}>{widgetData.phone}</Box>
						</Item>
					</Grid> }
					{ widgetData.email &&<Grid item xs={6}>
						<Item>
							<MailIcon fontSize="15"></MailIcon>
							<Box component="span" sx={{ pl: 1 }}>{widgetData.email}</Box>
						</Item>
					</Grid> }
					{ widgetData.website && <Grid item xs={6}>
						<Item>
							<LanguageIcon fontSize="15"></LanguageIcon>
							<Box component="span" sx={{ pl: 1 }}>{widgetData.website}</Box>
						</Item>
					</Grid> }
					{ widgetData.address && <Grid item xs={6}>
						<Item>
							<HomeIcon fontSize="15"></HomeIcon>
							<Box component="span" sx={{ pl: 1 }}>{widgetData.address}</Box>
						</Item>
					</Grid> }
				</Grid>
			</div>
			...
			{
				openEditorName === props.componentItem.name ? <BasicInfoEditor setWidgetData={setWidgetData} open={open} setOpen={setOpen} componentColumn={props.componentColumn} componentName={props.componentItem.name} editorData={widgetData} /> : null
			}
		</div>
	);
}
